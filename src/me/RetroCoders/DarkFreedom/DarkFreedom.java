...
import me.RetroCoders.DarkFreedom.AV_Util;
import me.RetroCoders.DarkFreedom.AbilityManager;
import me.RetroCoders.DarkFreedom.AvalancheFreedom;
  ...
  implements Listener
{
  ...
  {
    for (Player p : AvalancheFreedom.server.getOnlinePlayers()) {
    ...
    AvalancheFreedom.logger.info("[AdminWorld] " + player.getName() + ": " + s);
  }
  ...
  {
    Player player = event.getPlayer();
    if (AvalancheFreedom.rollbacksEnabled)
    {
      event.setCancelled(true);
  ...
  {
    Player player = event.getPlayer();
    if ((AvalancheFreedom.lockdownMode) && (!AV_Util.isSuperAdmin(player))) {
